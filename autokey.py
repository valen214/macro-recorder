...
def on_key_press(vk, func=None, modifiers=0):
    index = random.randrange(65536) # just hope it won't collide
    user32 = ctypes.windll.user32
    user32.RegisterHotKey(None, index, modifiers, vk)
    ...
    try:
        ...
        while user32.GetMessageA(ctypes.byref(msg), None, 0, 0) != 0:
            cont = None
            if msg.message == WM_HOTKEY and msg.wParam == vk:
                if func:
                    cont = func()
...
WindowInfo = namedtuple('WindowInfo', 'hWnd pid title')
...
def get_window(target):
    ...
    def enum_proc(hWnd, lParam):
        if user32.IsWindowVisible(hWnd):
            ...
            result.append(WindowInfo(hWnd, pid.value, title.value))
        return True
    user32.EnumWindows(WNDENUMPROC(enum_proc), 0)
    for hWnd, pid, title in result:
        if target in title.replace(" ", "").lower():
...
def main1():
    sleep(2)
    try:
        ret = user32.SetActiveWindow(get_window("monsterhunter"))
        print("success" if ret else "failed", kernel32.GetLastError())
        while True:
            # SendInput(Keyboard(32, KEYEVENTF_EXTENDEDKEY))
            SendInput(Keyboard(VK_SPACE))
            sleep(0.2)
            # SendInput(Keyboard(32, KEYEVENTF_EXTENDEDKEY | KEYEVENTF_KEYUP))
...
def main4():
    ...
    try:
        while True:
            ...
            arr = PBYTE256()
            arr[VK_SPACE] = 0xff
            user32.SetKeyboardState(ctypes.byref(arr))
    except KeyboardInterrupt:
        pass
...
def main():
    ...
    try:
        while True:
            sleep(0.5)
            user32.keybd_event(KEY_A, 0x45, 0, 0);
            sleep(0.5)
            user32.keybd_event(KEY_A, 0x45, KEYEVENTF_KEYUP, 0);
    except KeyboardInterrupt:
        pass
...
def main2():
    ...
    ret = user32.SendMessageA(hWnd, 0x0102, VK_SPACE, 0)
    print("success" if ret else "failed", kernel32.GetLastError())
    sleep(0.2)
    user32.SendMessageW(hWnd, 0x0100, VK_SPACE, 0)
    print("success" if ret else "failed", kernel32.GetLastError())
...
def main1():
    ...
    print("starting pressing space")
    arr = [Keyboard(32, KEYEVENTF_EXTENDEDKEY) for i in range(30)]
    SendInput(*arr)
    print(user32.MapVirtualKeyA(VK_SPACE, 2))
    try:
        # SendInput(Keyboard(VK_SPACE, 0))
        arr = PBYTE256()
        last = arr[:]
        while True:
            arr = PBYTE256()
            user32.GetKeyboardState(ctypes.byref(arr))
        ...
        last_state = user32.GetAsyncKeyState(VK_ESCAPE)
        while last_state == user32.GetAsyncKeyState(VK_ESCAPE):
            sleep(0.2)
    except KeyboardInterrupt:
        print("keyboard interrupt")
    else:
        print("received terminating signal")
